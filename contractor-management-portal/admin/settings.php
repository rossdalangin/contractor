...
function cmp_register_settings() {
    ...
    // QuickBooks Section
    add_settings_section(
        'cmp_quickbooks_section',
        'QuickBooks API Settings',
        'cmp_quickbooks_section_callback',
        'cmp-settings'
    );
    add_settings_field(
        'cmp_quickbooks_client_id',
        'QuickBooks Client ID',
        'cmp_quickbooks_client_id_callback',
        'cmp-settings',
        'cmp_quickbooks_section'
    );
    add_settings_field(
        'cmp_quickbooks_client_secret',
        'QuickBooks Client Secret',
        'cmp_quickbooks_client_secret_callback',
        'cmp-settings',
        'cmp_quickbooks_section'
    );
}
...
function cmp_sanitize_options( $input ) {
    ...
    if ( isset( $input['quickbooks_client_id'] ) ) {
        $output['quickbooks_client_id'] = sanitize_text_field( $input['quickbooks_client_id'] );
    }
    if ( isset( $input['quickbooks_client_secret'] ) ) {
        $output['quickbooks_client_secret'] = sanitize_text_field( $input['quickbooks_client_secret'] );
    }
...
function cmp_google_drive_section_callback() {
    ...
    $client = cmp_get_google_client();
    if ( ! $client ) {
        echo '<p class="description" style="color: red;">' . __( 'Google PHP Client Library is not installed. Please install it to enable Google Drive integration.', 'contractor-management-portal' ) . '</p>';
        return;
    }
...
/**
 * QuickBooks section callback.
 */
function cmp_quickbooks_section_callback() {
    echo '<p>Enter your QuickBooks API credentials below. You can get these from your <a href="https://developer.intuit.com/" target="_blank">Intuit Developer account</a>.</p>';

    // We will add the authorization button here later, guarded by a class_exists check.
    if ( ! class_exists('QuickBooksOnline\API\DataService\DataService') ) {
        echo '<p class="description" style="color: red;">' . __( 'QuickBooks PHP SDK is not installed. Please install it to enable QuickBooks integration.', 'contractor-management-portal' ) . '</p>';
    } else {
        // Placeholder for the actual auth button and status
        echo '<p class="description" style="color: orange;">' . __( 'QuickBooks integration is ready to be configured.', 'contractor-management-portal' ) . '</p>';
    }
}

/**
 * QuickBooks field callbacks.
 */
function cmp_quickbooks_client_id_callback() {
    $options = get_option( 'cmp_options' );
    $client_id = isset( $options['quickbooks_client_id'] ) ? $options['quickbooks_client_id'] : '';
    echo '<input type="text" id="cmp_quickbooks_client_id" name="cmp_options[quickbooks_client_id]" value="' . esc_attr( $client_id ) . '" size="50" />';
}

function cmp_quickbooks_client_secret_callback() {
    $options = get_option( 'cmp_options' );
    $client_secret = isset( $options['quickbooks_client_secret'] ) ? $options['quickbooks_client_secret'] : '';
    echo '<input type="text" id="cmp_quickbooks_client_secret" name="cmp_options[quickbooks_client_secret]" value="' . esc_attr( $client_secret ) . '" size="50" />';
}
